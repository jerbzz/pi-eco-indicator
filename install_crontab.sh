...
if [ "$(crontab -l | wc -c)" -eq 0 ]; then
  if [ -z "$1" ]; then
    ...
    if [[ "$1" =~ ^(A|B|C|D|E|F|G|P|N|J|H|K|L|M)$ ]]; then
      ...
      (crontab -l 2>/dev/null; echo "30 16 * * * cd /home/pi/agile-blinkt-indicator && /usr/bin/python3 store_prices.py --region $1 > ./blinkt.log 2>&1") | crontab -
      (crontab -l 2>/dev/null; echo "30 18 * * * cd /home/pi/agile-blinkt-indicator && /usr/bin/python3 store_prices.py --region $1 > ./blinkt.log 2>&1") | crontab -
      (crontab -l 2>/dev/null; echo "30 20 * * * cd /home/pi/agile-blinkt-indicator && /usr/bin/python3 store_prices.py --region $1 > ./blinkt.log 2>&1") | crontab -;
      echo "Done."
    else
